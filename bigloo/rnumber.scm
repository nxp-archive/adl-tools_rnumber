(module rnumber
  (export *rnumber-version* rnumber-ctor rnumber-cstr rnumber-equal)
  (eval (export-exports))
;  (eval (export rnumber-ctor) 
;	(export rnumber-cstr))
  (extern
...
(define (rnumber-ctor args)
  (cond ((null? args) (bl-rnumber-create))
	((integer? args) ((bl-rnumber-create-from-unsigned args)))
	(else (error "rnumber-ctor" "unknown args type " args))))

(define (rnumber-cstr args)
  (cond ((RNumber_proxy_t? args) (bl-rnumber-cstr args))
	(else (error "rnumber-cstr" "unknown args type " args))))

(define (rnumber-equal args)
  (if (pair? args)
      (let ((first-rn (RNumber_proxy_t? (car args)))
	    (first-ui (integer? (car args))))
	(if (pair? (cdr args))
	    (print "cdr is pair - cadr is " (cadr args))
	    (print "cdr is NOT pair - cdr is " (cdr args)))
	(bl-rnumber-rn-equal-rn (car args) (cadr args)))
      (error "rnumber-equal" "arguments needs to be a pair" args)))
