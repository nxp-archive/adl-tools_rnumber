...
pkgincludedir = $(includedir)/RNumber
...
OBJDUMP = @OBJDUMP@
PACKAGE = RNumber
PERLPROG = /local/tdukes/bin/perl
PROFILE = -fPIC
RANLIB = ranlib
RNUMBER_BINARY_AGE = 3
RNUMBER_INTERFACE_AGE = 3
RNUMBER_MAJOR_VERSION = 1
RNUMBER_MICRO_VERSION = 4
RNUMBER_MINOR_VERSION = 4
RNUMBER_VERSION = 1.4.4
VERSION = 1.4.4
YACC = bison -y

MACROS =  	std-exceptions.m4


EXTRA_DIST = $(m4data_DATA)

m4datadir = $(datadir)/aclocal
m4data_DATA = rnumber.m4

SUBDIRS =  	macros 	doc   	src   	tests 
...
CONFIG_CLEAN_FILES =  rnumber-config
SCRIPTS =  $(bin_SCRIPTS)
...
ltmain.sh missing mkinstalldirs rnumber-config.in
...
rnumber-config: $(top_builddir)/config.status rnumber-config.in
...
# This is run after the distribution directory is created
# This target is here to remove src/rcparser.yy from the dist
dist-hook:
	rm $(distdir)/src/rvparser.yy
